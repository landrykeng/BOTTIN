...
def show():
    ...
    with col_form:
        ...
        with st.form("alerte_form"):
            ...
            email_alerte = st.text_input("Email de notification", value=f"{st.session_state.get('username','')}@exemple.cm")
            newsletter = st.checkbox("S'abonner à la newsletter mensuelle du marché")

            submitted = st.form_submit_button(":material/notifications_active: Activer cette alerte", type="primary", use_container_width=True)
            if submitted:
                st.success(":material/check_circle: Alerte configurée avec succès ! Vous recevrez des notifications à l'adresse indiquée.")

    with col_actives:
        st.markdown("### Mes alertes actives")
        st.markdown("""
        <div class="app-card" style="margin-bottom:0.5rem;">
            <div style="font-weight:600; color:#003366; font-size:0.88rem;">:material/notifications: BTA · Cameroun · ≥ 3,5%</div>
            <div style="font-size:0.78rem; color:#8C8C8C; margin-top:3px;">Configurée le 2026-01-10</div>
        </div>
        <div class="app-card" style="margin-bottom:0.5rem;">
            <div style="font-weight:600; color:#003366; font-size:0.88rem;">:material/notifications: OTA 5 ans · Gabon · ≥ 7%</div>
            <div style="font-size:0.78rem; color:#8C8C8C; margin-top:3px;">Configurée le 2026-02-15</div>
        </div>
        """, unsafe_allow_html=True)

        st.info(":material/mail: **Newsletter** : vous êtes abonné. Prochain envoi : 1er mai 2026.")
